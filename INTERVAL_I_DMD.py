# -*- coding: utf-8 -*-
"""
Created on Wed Jul 28 22:38:35 2021
...
def main(filename: Path,Rey, start_n: int = 3000,end_n: int =15000,r:int =7):
    ...
    DMD = mr.compute_DMD_arrays_snaps_method(x,inner_product_weights=W)
    dT = 0.001
   ...
   # DMD Spectrum Graph
    ...
    for idx,FREQ_i in enumerate(FREQS):
        if FREQ_i  > (0 + noise) and FREQ_i < FREQS[fundamental_arg]: 
            fundamental_arg = idx
...
if __name__ == "__main__":
   ...
   # nu =  (0.1*U)/np.arange(81,86,1)
    ...
    for i in range(len(nu)):
        ...
        with open(str(os.path.dirname(__file__))+'\INTERVAL_I_SUBCRIT_JSON'+'\GRATES_DMD.json', 'w') as f:
                # indent=2 is not needed but makes the file human-readable
                json.dump(GRATES_DMD_REAL, f, indent=2) 
        with open(str(os.path.dirname(__file__))+'\INTERVAL_I_SUBCRIT_JSON'+'\FREQS_DMD.json', 'w') as f:
                # indent=2 is not needed but makes the file human-readable
