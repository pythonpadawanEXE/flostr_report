...
def main(
    filename: Path, mass: Path, start_n: int = 14000, end_n: int = 15000, r: int = 21
):
    W = load_npz(mass)
    ...
    for i, (u, color) in enumerate(zip(np.real_if_close(reconstruction), colors)):
        ...
        axes[i//2].yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
        axes[i//2].set_ylabel("Chronos Pair {}".format(i//2 + 1))
        axes[i//2].set_xticklabels([int(i) for i in (axes[i//2].get_xticks()+start_n)])
    axes[0].hlines(
        uinf,
        xmin=0,
...
if __name__ == "__main__":
    ...
    for i in range(len(nu)):
        ...
        parser.add_argument(
        "-m", "--mass", type=Path, default=Path(str(os.path.dirname(__file__))+'\\'+'SIM_XDMF\\'
                                   +"st08_navier_stokes_cylindermass"+
                                   "_Re_{}.npz".format(str(Re).replace('.','-')))
        )   
