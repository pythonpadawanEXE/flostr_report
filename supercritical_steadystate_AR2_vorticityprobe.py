...
def main(filename: Path,Re, start_n: int = 10000,end_n: int =15000,probetype= None,vorticity = None):
            ...
            if not os.path.exists(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'):
                try:
                    ...
                    if e.errno != errno.EEXIST:
                        raise
            if chronosi == 0:
                with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\GRATES_FORE.json', 'w') as f:
                        # indent=2 is not needed but makes the file human-readable
                        json.dump(GRATES_FORE, f, indent=2) 
                with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\FREQS_FORE.json', 'w') as f:
                        # indent=2 is not needed but makes the file human-readable
                        json.dump(FREQS_FORE, f, indent=2) 
                
            elif chronosi == 1:
                with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\GRATES_AFT.json', 'w') as f:
                        # indent=2 is not needed but makes the file human-readable
                        json.dump(GRATES_AFT, f, indent=2) 
                with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\FREQS_AFT.json', 'w') as f:
                        # indent=2 is not needed but makes the file human-readable
