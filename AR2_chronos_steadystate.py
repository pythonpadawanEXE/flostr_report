...
def main(filename: Path,Re,chronosi :int, start_n: int = 10000,end_n: int =15000,r:int = 1):
        ...
        if not os.path.exists(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'):
            try:
                ...
                if e.errno != errno.EEXIST:
                    raise
        if chronosi == 0:
            with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\GRATES_CHRON1.json', 'w') as f:
                    # indent=2 is not needed but makes the file human-readable
                    json.dump(GRATES_CHRON1, f, indent=2) 
            with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\FREQS_CHRON1.json', 'w') as f:
                    # indent=2 is not needed but makes the file human-readable
            ...
            save = open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\ROOTS_CHRON1.dat',"w")
            np.savetxt(save, ROOTS_CHRON1 , newline = "\r\n")
            save.close()
            
        elif chronosi == 1:
            with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\GRATES_CHRON2.json', 'w') as f:
                    # indent=2 is not needed but makes the file human-readable
                    json.dump(GRATES_CHRON2, f, indent=2) 
            with open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\FREQS_CHRON2.json', 'w') as f:
                    # indent=2 is not needed but makes the file human-readable
            ...
            save = open(str(os.path.dirname(__file__))+'\SS_SCRIT_JSON'+'\ROOTS_CHRON2.dat',"w")
